 ...
 * @param {*} props
 */
export default withAuth(function CreateEventForm(props) {
  /**
   * setup state hooks
   */
  ...
  function isValidFormData(formData) {
    ...
    if (selectedStartDate.getTime < new Date()) {
      alert("selected date must be after current time");
      return false;
    }
                ...
                <div className="col-4 offset-2">
                  <MuiPickersUtilsProvider utils={DateFnsUtils}>
                    <DateTimePicker
                      ...
                      label="Start Date"
                      onChange={(_, newValue) =>
                        selectedStartDateChange(newValue)
                      }
                      onAccept={date => {
                        selectedStartDateChange(date);
                      }}
                      required
                    />
